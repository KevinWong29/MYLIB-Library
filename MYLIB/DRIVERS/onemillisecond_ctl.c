...
static inline void SetCompareValue(void)
{
    ...
    {
        HWREG16(TIMERA_TACCR1_REG_ADDR) = currentValue + ONEMS__TIMER_COMPARE_VALUE;
    }    
    else
    {
        //Calculate the compare value when the counter loops back
        HWREG16(TIMERA_TACCR1_REG_ADDR) = ONEMS__TIMER_COMPARE_VALUE - (0xFFFF - currentValue);
    }    
}
...
void ONEMS__Reset(void) 
{
    ...
    HWREG16(TIMERA_TACCTL1_REG_ADDR) = ONEMS__TACCTL_STARTUP_CONFIG;
    HWREG16(TIMERA_TACCTL1_REG_ADDR) &= ~TIMERA_CCIFG_MASK;

    //Load counter compare value to compare and generate interrupt
    HWREG16(TIMERA_TACCR1_REG_ADDR) = ONEMS__TIMER_COMPARE_VALUE;
